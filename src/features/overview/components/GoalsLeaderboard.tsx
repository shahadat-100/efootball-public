...
// Goals from aggregated season stats (for Overall)
const calcSeasonGoals = (stats: PlayerSeasonStat[]): number =>
  stats.reduce((total, s) => total + (s.goals || 0), 0);

// Goals from individual match entries (for Weekly / Monthly)
const calcEntryGoals = (entries: MatchEntry[]): number =>
  entries.reduce((total, e) => total + (e.goals || 0), 0);
...
export function GoalsLeaderboard({ players, matchEntries, seasons, playerSeasonStats, limit }: GoalsLeaderboardProps) {
  // ── Monthly filters ──
  ...
  const { weeklyRanking, monthlyRanking, overallRanking } = useMemo(() => {
    ...
    matchEntries.forEach(entry => {
      ...
      const gls = calcEntryGoals([entry]);
      ...
      if (isCurrentWeek && weeklyMap.has(entry.playerId)) {
        weeklyMap.set(entry.playerId, weeklyMap.get(entry.playerId)! + gls);
      }
      ...
      if (isMonthMatch && isSeasonOrYearMatch && monthlyMap.has(entry.playerId)) {
        monthlyMap.set(entry.playerId, monthlyMap.get(entry.playerId)! + gls);
      }
    });
    ...
    players.forEach(p => {
      ...
      overallMap.set(p.id, calcSeasonGoals(stats));
    });
  ...
  const renderRow = (r: RankedPlayer, i: number) => (
    <div key={r.player.id} className={cn(
      ...
      <div className={cn(
        ...
        r.goals > 0 ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" :
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
      ...
      <CardWrapper accentColor="#ef4444">
        <div className="mb-4 border-b border-border pb-3 flex items-center justify-between relative z-10">
          <h3 className="font-bold text-base text-foreground tracking-tight">Weekly Goals</h3>
          <span className="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-red-500/10 text-red-600 border border-red-500/20 shadow-sm">
            {weeklyRanking.name}
          </span>
        </div>
        <div className="flex-1 overflow-y-auto pr-1 space-y-1 custom-scrollbar relative z-10">
          {weeklyRanking.list.length === 0 || weeklyRanking.list.every(r => r.goals === 0) ? (
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                <span className="text-3xl mb-2 block">⚽</span>
                <p className="text-muted-foreground text-sm font-medium">No goals this week</p>
              </div>
            </div>
          ) : (
      ...
      <CardWrapper accentColor="#06b6d4">
        <div className="mb-4 border-b border-border pb-3 relative z-10">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-bold text-base text-foreground tracking-tight">Monthly Goals</h3>
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-1 custom-scrollbar relative z-10">
          {monthlyRanking.list.length === 0 || monthlyRanking.list.every(r => r.goals === 0) ? (
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                <span className="text-3xl mb-2 block">⚽</span>
                <p className="text-muted-foreground text-sm font-medium">No goals for this period</p>
              </div>
            </div>
          ) : (
      ...
      <CardWrapper accentColor="#f59e0b">
        <div className="mb-4 border-b border-border pb-3 flex items-center justify-between relative z-10">
          <h3 className="font-bold text-base text-foreground tracking-tight">Overall Goals</h3>
          <select
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-1 custom-scrollbar relative z-10">
          {overallRanking.list.length === 0 || overallRanking.list.every(r => r.goals === 0) ? (
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                <span className="text-3xl mb-2 block">🏆</span>
