...
export function TopScorerSpotlights({ players, playerSeasonStats, playerWeeklyStats = [], playerMonthlyStats = [] }: TopScorerProps) {
  const { weekTop, monthTop, seasonTop } = useMemo(() => {
    ...
    const getTop = (map: Map<string, { pts: number; goals: number; motm: number }>) => {
      const sorted = Array.from(map.entries())
        ...
        .sort((a, b) => b.goals - a.goals || b.pts - a.pts);
  ...
  return (
    <div className="mb-12 stagger-children">
      <h3 className="font-heading font-bold text-[22px] tracking-tight mb-4 flex items-center gap-2">
        <SparklesIcon className="text-amber-500" /> Top Scorers
      </h3>
      <div className="flex flex-col md:flex-row gap-6">
        {weekTop.data && (
          <SpotlightCard title="Top Scorer of the Week" subtitle={weekTop.label} data={weekTop.data} type="week" />
        )}
        {monthTop.data && (
          <SpotlightCard title="Top Scorer of the Month" subtitle={monthTop.label} data={monthTop.data} type="month" />
        )}
        {seasonTop.data && (
          <SpotlightCard title="Top Scorer of the Season" subtitle={seasonTop.label} data={seasonTop.data} type="season" />
        )}
      </div>
    </div>
  );
