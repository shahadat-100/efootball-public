      ...
      <div className="flex flex-col gap-3 md:hidden">
        {isEmpty ? (
          ...
          pageList.map(r => {
            ...
            const rank1Points = activeListForRank[0]?.points ?? 0;
            const rank2Points = activeListForRank[1]?.points ?? 0;
            const isCloseChaser = isRank2 && (rank1Points - rank2Points) <= 3;

            return (
              <div key={r.player.id} className="relative pt-2">
                {/* 💬 Challenge Speech Bubble for #1 Leader */}
                {isRank1 && (
                  <div className="absolute -top-3 left-10 z-20 animate-bounce">
                    <div className="relative border-2 border-amber-500/60 bg-gradient-to-r from-amber-950 via-zinc-900 to-amber-950 px-3 py-1 text-amber-300 shadow-md rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1 border-amber-500/80">
                ...
                {/* 💬 Chaser Speech Bubble for #2 when gap ≤ 3 pts */}
                {isCloseChaser && (
                  <div className="absolute -top-3 left-10 z-20 animate-bounce">
                ...
                <div
                  ...
                  className={cn(
                    ...
                    activeRank === 0 ? "bg-gradient-to-r from-amber-500/10 via-card to-card border-amber-500/40 shadow-md" :
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="flex flex-col items-center justify-center shrink-0 w-9 h-9 rounded-xl bg-background/80 border border-border/60 shadow-xs">
                        <span className={cn(
                          ...
                          activeRank === 0 ? "text-amber-500 text-sm" :
                      ...
                      <div className="min-w-0">
                        <h4 className={cn("font-bold text-foreground truncate text-sm leading-tight flex items-center gap-1.5", isTop3 && "text-amber-500 font-extrabold")}>
                          {r.player.name}
                          {activeRank === 0 && <Crown className="w-3.5 h-3.5 text-amber-500 inline shrink-0" />}
                        </h4>
                    ...
                    <div className="shrink-0 text-right">
                      <span className={cn(
                        ...
                        {r.points > 0 ? `+${r.points}` : r.points} <span className="text-[10px] font-semibold opacity-70">PTS</span>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5 pt-3 mt-3 border-t border-border/40 text-center text-xs">
                    ...
                    <div className="bg-muted/20 border border-border/30 px-1 py-1 rounded-lg">
                      <span className="text-[8.5px] font-bold text-muted-foreground uppercase block tracking-wider">GF/GC</span>
